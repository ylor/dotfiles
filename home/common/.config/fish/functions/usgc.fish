...
function pad_name
    ...
    if test $name_len -lt $MIN_NAME_LEN
        ...
        set -l cut_end (math $MAX_NAME_LEN - 1)
...
function PRINT_CENTERED_DATA
    ...
    set -l padding_left (math -s 0 "($total_width - $text_len) / 2")
...
function PRINT_DATA
    set -l name (pad_name $argv[1])
    set -l data $argv[2]
    set -l max_data_len $CURRENT_LEN

    # Truncate or pad data
    set -l data_len (string length -- $data)
    if test $data_len -gt $MAX_DATA_LEN; or test $data_len -eq (math $MAX_DATA_LEN - 1)
        set -l cut_end (math $MAX_DATA_LEN - 1)
...
function bar_graph
    ...
    set -l num_blocks (awk -v percent=$percent -v width=$width 'BEGIN { printf "%d", (percent / 100) * width }')
...
if test -f /etc/os-release
    set -l NAME (grep "^NAME=" /etc/os-release | cut -d'=' -f2 | tr -d '"')
    debug "OS: $NAME"
    set -l VERSION (grep "^VERSION=" /etc/os-release | cut -d'=' -f2)
    set -l VERSION_CODENAME (grep "^VERSION_CODENAME=" /etc/os-release | cut -d'=' -f2)
...
switch (uname)
    case Linux
        set -g cpu_freq (grep 'cpu MHz' /proc/cpuinfo | cut -f 2 -d ':' | awk 'NR==1 { printf "%.2f", $1 / 1000 }') # Convert from M to G units
    case Darwin
        set -g cpu_freq (sysctl -n hw.cpufrequency | awk 'NR==1 { printf "%.2f", $1 / 1000000000 }') # Convert from Hz to GHz units
    case FreeBSD
        set -g cpu_freq (sysctl -n dev.cpu.0.freq | awk 'NR==1 { printf "%.2f", $1 / 1000 }') # Convert from M to G units
    case '*'
        set -g cpu_freq "???"
end
...
switch (uname)
    ...
    case FreeBSD
        set -l physmem (sysctl -n hw.memsize)
        set -g mem_total (math -s 0 "$physmem / 1024")
        set -l free_count (sysctl -n vm.stats.vm.v_free_count)
        set -l inactive_count (sysctl -n vm.stats.vm.v_inactive_count)
        set -l pagesize (sysctl -n hw.pagesize)
        set -g mem_available (math -s 0 "($free_count + $inactive_count) * $pagesize / 1024")
    case Darwin
        set -l physmem (sysctl -n hw.memsize)
        set -g mem_total (math -s 0 "$physmem / 1024")
        set -l pagesize (sysctl -n hw.pagesize)
        set -l pages_free (vm_stat | grep "Pages free:" | tr -d " ." | cut -d ":" -f 2)
        set -l pages_inactive (vm_stat | grep "Pages inactive:" | tr -d " ." | cut -d ":" -f 2)
        set -l pages_speculative (vm_stat | grep "Pages speculative:" | tr -d " ." | cut -d ":" -f 2)
        set -l pages_purgeable (vm_stat | grep "File-backed pages:" | tr -d " ." | cut -d ":" -f 2)
...
set -g mem_percent (printf "%.2f" $mem_percent)
set -g mem_total_gb (echo $mem_total | awk '{ printf "%.2f", $1 / (1024 * 1024) }') # (From Ki to Gi units)
# set -g mem_available_gb (echo $mem_available | awk '{ printf "%.2f", $1 / (1024 * 1024) }') # (From Ki to Gi units) Not used currently
set -g mem_used_gb (echo $mem_used | awk '{ printf "%.2f", $1 / (1024 * 1024) }')
...
if command -v zfs >/dev/null 2>&1; and test (zpool list -H | wc -l | string trim) -gt 0
    ...
    set -g zfs_available_gb (echo $zfs_available | awk '{ printf "%.2f", $1 / (1024 * 1024 * 1024) }') # (To G units)
    set -g zfs_used_gb (echo $zfs_used | awk '{ printf "%.2f", $1 / (1024 * 1024 * 1024) }') # (To G units)
    ...
    set -l root_partition "/"
    set -g root_used (df -m $root_partition | awk 'NR==2 {print $3}')
    set -g root_total (df -m $root_partition | awk 'NR==2 {print $2}')
...
if command -v lastlog >/dev/null 2>&1
    ...
    set -l last_login (lastlog -u $USER | string collect)
    ...
    switch (uname)
        ...
        case '*'
            set -l fields (last $USER | head -n 1 | awk '{for(i=1;i<=NF;i++) print $i}')
            set -l f3 $fields[3]
...
set -q SSH_TTY && PRINT_DATA "CLIENT  IP" $net_client_ip

# TODO: verify
PRINT_DATA "DNS IP" (string join ", " $net_dns_ip)
